...
if (typeof document !== "undefined") {
  ...
  syncCookie()
  pb.authStore.onChange(syncCookie, true)
}
...
export const getPublications = async (page = 1, perPage = 20) => {
  try {
    ...
    // For each publication, fetch its comment count
    const itemsWithCount = await Promise.all(
      records.items.map(async (pub) => {
        const comments = await pb.collection("comments").getList(1, 1, {
          filter: `publication = "${pub.id}"`,
          fields: "id",
        })
        return { ...pub, comments_count: comments.totalItems }
      })
    )

    return { success: true, data: { ...records, items: itemsWithCount } }
  } catch (error) {
    return { success: false, error: error.message }
  }
}
...
export const getComments = async (publicationId) => {
  try {
    const records = await pb.collection("comments").getList(1, 500, {
      expand: "user,parent_comment.user",
      sort: "created",
      filter: `publication = "${publicationId}"`,
    })
    return { success: true, data: records }
...
export const createComment = async (commentData) => {
  try {
    ...
    await pb.collection("publications").update(commentData.publication, {
      "comments_count+": 1,
    })
...
export const createCommentReply = async (publicationId, userId, content, parentCommentId) => {
  try {
    ...
    await pb.collection("publications").update(publicationId, {
      "comments_count+": 1,
    })
...
export const deleteComment = async (commentId) => {
  try {
    // get the publication id before deleting
    const comment = await pb.collection("comments").getOne(commentId)
    const publicationId = comment.publication
...
// Membership
export const createMembership = async (userId, plan = "trial", durationDays = 30) => {
  try {
    ...
    const record = await pb.collection("membership").create({
      user: userId,
      plan: plan,
...
export const getUserMembership = async (userId) => {
  try {
    const records = await pb.collection("membership").getList(1, 1, {
      filter: `user = "${userId}"`,
      sort: "-created",
    })

    if (records.items.length > 0) {
      return { success: true, data: records.items[0] }
    }
...
export const checkUserMembership = async (userId) => {
  try {
    ...
    if (!result.success) {
      return { success: false, error: result.error, isMember: false }
    }

    const membership = result.data

    if (!membership) {
      return { success: true, isMember: false, data: null }
    }

    const today = new Date().toISOString().split("T")[0]
    const isActive = membership.end_date >= today
